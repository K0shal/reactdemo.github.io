...
function MapLocator(){
    ...
    for (var i = 0; i < store.length; i++) {
      let l=distance(location.latitude, location.longitude, store[i].latitude, store[i].longitude, "K")
        if (d<l) {d=l;
...
>
  ...
  {store.map((el)=>{
      if(el!=nearStore)
       return<Marker position={[el.latitude, el.longitude]} icon={icon}>
       <Popup>Too Far</Popup>
     </Marker>;
  })}
  <Marker position={[nearStore.latitude, nearStore.longitude]} icon={icon}>
